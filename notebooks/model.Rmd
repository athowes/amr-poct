...
Assume some proportion of total neonates receive antibiotics, and that the remainder of those not perscribed to high risk sepsis neonates are perscribed to low risk sepsis neonates:
...
To run the sensitivity analysis, we create a function to run the cost effectiveness analysis above:
...
run_cea <- function(sensitivity, specificity, amr_burden_per_abx = NULL) {
  ...
  # If no AMR burden per perscription provided then calculate one
...
Check that it produces the same answer as above:
