...
## Varying AMR burden per perscription

```{r}
grid <- seq(from = 0, to = 0.5, length.out = 51)
df <- tidyr::crossing(sensitivity = 0.9, specificity = 0.9, amr_burden_per_abx = grid)
df <- bind_cols(df, pmap_dfr(df, run_cea))
```

```{r}
df %>%
  ggplot(aes(x = amr_burden_per_abx, y = 1 / icer)) +
  geom_line() +
  geom_hline(yintercept = 0, col = "lightgrey", linetype = "dashed") +  
  theme_minimal() +
  labs(x = "AMR burden per perscription", y = "DALYs per dollar", caption = "Sensitivity and specificity fixed to 90%")

df %>%
  ggplot(aes(x = amr_burden_per_abx, y = benefit_incremental)) +
  geom_line() +
  geom_hline(yintercept = 0, col = "lightgrey", linetype = "dashed") +
  theme_minimal() +
  scale_y_continuous(labels = scales::unit_format(unit = "M", scale = 10E-6)) +
  labs(x = "AMR burden per perscription", y = "Incremental benefit (DALYs)", caption = "Sensitivity and specificity fixed to 90%")
```
